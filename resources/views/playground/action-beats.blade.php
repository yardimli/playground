<?php
	...
	if (isset($_POST['book']) && !empty($_POST['book'])) {
		...
		echo json_encode(['success' => false, 'message' => __('Book not found.')]);
		exit();
	}
	...
	if (file_exists($jsonFilePath)) {
		...
		if (json_last_error() !== JSON_ERROR_NONE) {
			echo json_encode(['success' => false, 'message' => __('Error decoding JSON:') . json_last_error_msg()]);
			exit();
		}
	} else {
		echo json_encode(['success' => false, 'message' => __('Chapters JSON file not found.')]);
		exit();
	}
	...
	if (file_exists($bookJsonPath)) {
		...
		echo json_encode(['success' => false, 'message' => __('Book JSON file not found.')]);
	}
	...
	if ($_SERVER['REQUEST_METHOD'] === 'POST') {
		...
		switch ($action) {
			...
			//-----------------------------//
			case 'write_beats':
				if ($current_user === __('Visitor')) {
					echo json_encode(['success' => false, 'message' => __('You must be logged in to create a book.')]);
					break;
				}

				if ($bookData['owner'] !== $current_user) {
					echo json_encode(['success' => false, 'message' => __('You are not the owner of this book.')]);
					break;
				}
				...
				$chapterFromPreviousChapter = $_POST['chapterFromPreviousChapter'];
				...
				if ($simulated) {
					...
					if ($use_llm === 'anthropic-haiku' || $use_llm === 'anthropic-sonet') {
						...
						$prompt = str_replace('##previous_chapter##', $chapterFromPreviousChapter, $prompt);
					...
					if (isset($resultData['beats'])) {
						...
						echo json_encode(['success' => false, 'message' => __('Failed to generate beats')]);
						break;
					}
			...
			case 'write_beat_text':
				...
				$previous_beat_summaries = $_POST['previous_beat_summaries'];
			...
			case 'save_beats':
				...
				if (file_exists($chapterFilePath)) {
					...
					if (file_put_contents($chapterFilePath, json_encode($chapterData, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE))) {
						...
						echo json_encode(['success' => false, 'message' => __('Failed to write to file')]);
					}
				} else {
					echo json_encode(['success' => false, 'message' => __('Chapter file not found')]);
				}
				break;
		}
	}
